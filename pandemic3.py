...
for i in range(n_iter):
    # Initialize iteration array - 50 days for 21 students | 0 = not infected, 1 = infected
    ...
    for j in range(50):
        ...
        for k in range(21):
            ...
            if np.sum(students[j,:])<=1:
                if np.random.uniform()<=pinf and np.sum(students[j,:])>=1 and np.sum(students[:,k])<1:
                    students[j+1:j+4,k]=1 #infection starts on next day and goes for 3 days total
            else:
                for m in range(int(np.sum(students[j,:]))):
                    if np.random.uniform()<=pinf and np.sum(students[j,:])>=1 and np.sum(students[:,k])<1:
                        students[j+1:j+4,k]=1 #infection starts on next day and goes for 3 days total
...
#write to csv
studentdf.to_csv("./data/nomultiternonnormstudentf50.csv")
studentdfstat.to_csv("./data/nomultiternormstudentf50.csv")
plotdf.to_csv('./data/nomultiternormsum50.csv')
